...
import { X, Calendar as CalendarIcon, MapPin, Building, Monitor, CheckCircle2, Bell, Link as LinkIcon, RotateCw, User, Users } from 'lucide-react'
...
export function CalendarEventPanel({ isOpen, onClose, defaultType = CalendarEventType.EVENT, workspaceSlug, onCreate, initialDate, canCreateEvents = true, userRole }: CalendarEventPanelProps) {
                                ...
                                <Select
                                    ...
                                    onValueChange={(val) => {
                                        if (scope === 'team') {
                                            ...
                                            if (!selectedMembers.includes(val)) setSelectedMembers([...selectedMembers, val])
                                        }
                                    }}
                                >
                                    <SelectTrigger className="w-full h-full border-none bg-transparent p-0 hover:bg-transparent focus:ring-0 focus:ring-offset-0 focus:outline-none shadow-none text-sm font-normal disabled:opacity-100 disabled:cursor-default">
                                    ...
                                    <SelectContent className="bg-[#1a1a24] border-gray-800 text-white max-h-[200px]">
                                        {scope === 'team' ? (
                                            ...
                                            teamMembers.length > 0 ? (
                                                teamMembers
                                                    .filter(member => !['member', 'guest'].includes(userRole || '') ? true : member.id === session?.user?.id)
                                                    .map((member) => (
                                                        <SelectItem
                                                            key={member.id}
                                                            value={member.id}
