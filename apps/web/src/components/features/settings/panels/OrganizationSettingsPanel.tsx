...
export function OrganizationSettingsPanel({ isOpen, onClose }: OrganizationSettingsPanelProps) {
    ...
    return createPortal(
        <>
            ...
            <div className={`fixed inset-0 sm:inset-auto sm:top-4 sm:right-4 sm:bottom-4 w-full sm:w-[560px] max-w-none sm:max-w-xl bg-[var(--app-bg-card)] border border-[var(--app-border)] rounded-none sm:rounded-2xl z-50 flex flex-col shadow-2xl transform transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : 'translate-x-full sm:translate-x-[calc(100%+2rem)]'
                }`}>
                {/* Header */}
