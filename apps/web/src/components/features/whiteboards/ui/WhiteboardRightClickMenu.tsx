...
import { Trash2, Copy, ClipboardPaste, Layers, Ungroup, ArrowUpToLine, ArrowDownToLine, CopyPlus } from "lucide-react";
...
export function WhiteboardRightClickMenu({ x, y, visible, onClose, excalidrawAPI, theme }: WhiteboardRightClickMenuProps) {
    ...
    const shortcutClass = cn("ml-auto text-[10px] font-semibold opacity-50", isDark ? "text-white/50" : "text-gray-400");
    ...
    const duplicateSelected = () => {
        ...
        selectedElements.forEach(el => idMap.set(el.id, `${el.id}-copy-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`));
        ...
        oldGroupIds.forEach(g => groupIdMap.set(g, `group-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`));
        ...
        const duplicated = selectedElements.map(el => {
            ...
            return {
                ...
                containerId: el.containerId && idMap.has(el.containerId) ? idMap.get(el.containerId) : el.containerId,
                frameId: el.frameId,
                updated: Date.now(),
    ...
    const groupSelected = () => {
        ...
        const next = elements.map(el => selectedIds.includes(el.id)
    ...
    const pasteSelected = () => {
        try {
            ...
            const idMap = new Map<string, string>();
            clipboard.forEach(el => idMap.set(el.id, `${el.id}-paste-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`));
    ...
    return (
        <div
            className={cn(
                ...
                isDark ? "border-white/10 bg-[#2d2d44]/98" : "border-black/5 bg-white/98"
            )}
            style={{ left: x, top: y }}
        ...
        >
            {hasSelection ? (
                <>
                    ...
                    <button onClick={deleteSelected} className={cn(menuItemClass, "text-red-400 hover:text-red-300")}>
