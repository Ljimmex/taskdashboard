...
export function getWorkspacePlanLimits(
    workspace: Pick<Workspace, 'subscriptionPlan'>
): PlanLimits {
...
export function getWorkspaceDefaultsForPlan(plan: SubscriptionPlan): Partial<{
    ...
    maxWhiteboards: number | null
    maxFileSizeMB: number
    features: ReturnType<typeof getWorkspacePlanLimits>['features'] & { apiAccess: 'none' | 'read_only' | 'full' }
}> {
    switch (plan) {
        case 'free':
            return {
