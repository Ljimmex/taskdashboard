...
import { type WorkspaceRole, canUploadFiles } from '../../lib/permissions'
import {
  checkWorkspaceStorageLimit,
  checkFileSizeLimit,
  getWorkspaceUsedStorageBytes,
  getWorkspaceStorageLimitGB,
...
app.post('/upload', zValidator('json', uploadSchema), async (c) => {
  ...
  // Enforce file upload permission
  if (!canUploadFiles(workspaceRole)) {
    return c.json({ error: 'Forbidden: You do not have permission to upload files' }, 403)
  }

  // Validate file type against the MIME/extension whitelist
  if (!isAllowedFile(body.name, body.mimeType)) {
    return c.json({ error: 'File type not allowed', code: 'INVALID_FILE_TYPE' }, 400)
  }
  ...
  if (newFile) {
    ...
    const members = await db.query.workspaceMembers.findMany({
      where: (wm, { eq, and }) =>
        and(eq(wm.workspaceId, body.workspaceId), eq(wm.status, 'active')),
      columns: { userId: true },
    })
...
app.get('/', zValidator('query', filesQuerySchema), async (c) => {
  ...
  const conditions = [eq(files.workspaceId, workspaceId), eq(files.isArchived, false)]
...
app.get('/:id/content', async (c) => {
  ...
  const isText =
    textTypes.some((t) => file.mimeType?.startsWith(t)) ||
    [
      'txt',
      'md',
      'csv',
...
app.post('/:id/duplicate', async (c) => {
  ...
  const workspaceRole = await getUserWorkspaceRole(user.id, originalFile.workspaceId || '')
  if (!workspaceRole) {
    return c.json({ error: 'Forbidden: No active workspace access' }, 403)
  }
