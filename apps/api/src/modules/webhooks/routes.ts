...
import { canManageWorkspaceSettings, type WorkspaceRole } from '../../lib/permissions'
...
// Helper: Get user's active workspace role
async function getUserWorkspaceRole(
  ...
  const member = await db.query.workspaceMembers.findFirst({
    where: (m, { and, eq }) =>
      and(eq(m.workspaceId, workspaceId), eq(m.userId, userId), eq(m.status, 'active')),
  })
...
webhooksRoutes.get('/', async (c) => {
  try {
    ...
    // Security Check: Verify user is an active member with settings access
...
webhooksRoutes.post('/', zValidator('json', createWebhookSchema), async (c) => {
  try {
    ...
    // Verify user can manage workspace settings
    const workspaceRole = await getUserWorkspaceRole(userId, body.workspaceId)
    if (!canManageWorkspaceSettings(workspaceRole)) {
...
webhooksRoutes.patch('/:id', zValidator('json', updateWebhookSchema), async (c) => {
  try {
    ...
    // 2. Check Permissions
    const workspaceRole = await getUserWorkspaceRole(userId, webhook.workspaceId)
    if (!canManageWorkspaceSettings(workspaceRole)) {
      return c.json({ success: false, error: 'Forbidden' }, 403)
    }
