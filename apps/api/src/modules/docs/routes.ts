...
import { checkWorkspaceDocumentLimit } from '../../lib/workspaceLimits'
...
docsRoutes.get('/', async (c) => {
  try {
    ...
    const result = await db.query.documents.findMany({
      where: (d, { eq, and }) => and(eq(d.workspaceId, workspaceId), eq(d.isArchived, false)),
      with: {
        creator: {
...
docsRoutes.post(
  ...
  zValidator('json', createDocSchema, (result, c) => {
    if (!result.success) {
      console.error('Zod Validation Error on createDocSchema:', result.error.issues)
      return c.json(
  ...
  async (c) => {
    try {
      ...
      const limitCheck = await checkWorkspaceDocumentLimit(workspaceId)
      ...
      const newDoc: NewDocument = {
        ...
        title: body.title,
        content: sanitizeDocumentContent(body.content) || {},
        createdBy: user.id,
      }
