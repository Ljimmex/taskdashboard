...
async function getUserPermissions(userId: string, teamId: string) {
  ...
  const workspaceMember = await db.query.workspaceMembers.findFirst({
    where: (wm, { and, eq }) =>
      and(eq(wm.userId, userId), eq(wm.workspaceId, teamMember.team.workspaceId)),
    columns: { role: true },
  })
  ...
  try {
    ...
    const events = await db.query.calendarEvents.findMany({
      where: (e, { and, gte, lte, eq, or, sql }) => {
        ...
        const teamCheck =
          allowedTeamIds.length > 0 ? sql`${e.teamIds} && ${pgArrayLiteral}::uuid[]` : sql`false`

        const personalCheck = or(eq(e.createdBy, userId), sql`${userId} = ANY(${e.attendeeIds})`)
    ...
    return c.json(
      { success: false, error: 'Failed to fetch events', details: String(error?.message || error) },
      500
    )
  }
})
...
calendarRoutes.get('/export/:workspaceSlug', async (c) => {
  try {
    ...
    const events = await db.query.calendarEvents.findMany({
      where: (e, { and, eq, sql }) =>
        and(eq(e.workspaceId, workspace.id), sql`team_ids && ${pgArrayLiteral}::uuid[]`),
    })
...
calendarRoutes.get('/:id', async (c) => {
  try {
    ...
    // Access check: creator, attendee, or member of one of the assigned teams
    ...
    const isTeamMember =
      ...
      (
        await db.query.teamMembers.findMany({
          where: (tm, { and, eq }) => and(eq(tm.userId, user.id), inArray(tm.teamId, teamIds)),
          columns: { teamId: true },
          limit: 1,
        })
      ).length > 0

    if (!isCreator && !isAttendee && !isTeamMember) {
  ...
  try {
    ...
    if (teamIds.length > 0) {
      ...
      const member = await db.query.workspaceMembers.findFirst({
        where: (wm, { and, eq }) =>
          and(eq(wm.userId, userId), eq(wm.workspaceId, workspaceId as string)),
      })
      ...
      if (!canAssignOthers) {
        ...
        if (otherAttendees.length > 0) {
          return c.json(
            { error: 'Forbidden: Members can only create personal reminders for themselves' },
            403
          )
        }
      }
      ...
      if (firstEvent && (firstEvent.type === 'meeting' || firstEvent.type === 'reminder')) {
        ...
        for (const attendeeId of attendees) {
          ...
          await NotificationService.push(attendeeId, {
            ...
            title:
              firstEvent.type === 'meeting'
                ? 'notifications.titles.meeting_invited'
                : 'notifications.titles.event_reminder',
            message:
              firstEvent.type === 'meeting'
...
calendarRoutes.patch('/:id', zValidator('json', updateEventSchema), async (c) => {
  try {
    ...
    if (!isAuthor && !canManage) {
      return c.json(
        { error: 'Forbidden: You can only edit your own events or need manage permission' },
        403
      )
    }
