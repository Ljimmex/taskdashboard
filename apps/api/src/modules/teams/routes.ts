...
import { canManageTeamMembers, type WorkspaceRole, type TeamLevel } from '../../lib/permissions'
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      const resolvedTeamName =
        teamName ||
        (teamSlug
          ? teamSlug.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())
          : null)
      ...
      const team = await tx.query.teams.findFirst({
        where: (table, { eq, and }) =>
          and(eq(table.workspaceId, ws.id), eq(table.name, resolvedTeamName)),
      })
      ...
      const existingTeamMember = await tx.query.teamMembers.findFirst({
        where: (tm, { eq, and }) => and(eq(tm.teamId, team.id), eq(tm.userId, userId)),
      })
...
teamsRoutes.delete('/:id', async (c) => {
  ...
  try {
    ...
    const canDelete =
      workspaceRole === 'owner' || workspaceRole === 'admin' || teamLevel === 'team_lead'
    if (!canDelete) {
...
teamsRoutes.post('/:id/members', zValidator('json', addTeamMemberSchema), async (c) => {
  ...
  try {
    ...
    const isWsMember = await db.query.workspaceMembers.findFirst({
      where: (wm, { eq, and }) =>
        and(eq(wm.workspaceId, team.workspaceId), eq(wm.userId, targetUserId)),
    })
    ...
    const newMember = await db.transaction(async (tx) => {
      ...
      const allProjectIds = Array.from(
        new Set([...teamProjects.map((p) => p.id), ...linkedProjects.map((p) => p.id)])
      )

      if (allProjectIds.length > 0) {
        // Check existing memberships to avoid duplicates
        const existingProjectMems = await tx
      ...
      for (const projectId of allProjectIds) {
        ...
        if (otherTeamIds.length > 0) {
          ...
          const inOtherTeam = await db.query.teamMembers.findFirst({
            where: (tm, { inArray, and, eq }) =>
              and(eq(tm.userId, memberId), inArray(tm.teamId, otherTeamIds)),
          })
    ...
    try {
      ...
      if (Array.isArray(body.teams)) {
        ...
        await db.delete(teamMembers).where(
          and(
            ...
            targetTeamIds.length > 0
              ? sql`team_id NOT IN (${sql.join(
                  targetTeamIds.map((t) => sql`${t}`),
                  sql`, `
                )})`
              : sql`true`
          )
        ...
        const currentMemberships = await db
          .select()
          .from(teamMembers)
          .where(and(eq(teamMembers.userId, memberId), inArray(teamMembers.teamId, targetTeamIds)))
      ...
      if (projectNames && Array.isArray(projectNames)) {
        ...
        const workspaceProjectData = await db
