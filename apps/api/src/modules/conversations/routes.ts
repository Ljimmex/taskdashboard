...
const messageContentSchema = z.union([zSanitizedString(), encryptedMessageSchema])
...
async function areWorkspaceMembers(workspaceId: string, userIds: string[]): Promise<boolean> {
  if (userIds.length === 0) return true
  const members = await db.query.workspaceMembers.findMany({
    where: (m, { and, eq }) =>
      and(eq(m.workspaceId, workspaceId), eq(m.status, 'active'), inArray(m.userId, userIds)),
    columns: { userId: true },
  })
  return members.length === userIds.length
}

async function areTeamMembers(teamId: string, userIds: string[]): Promise<boolean> {
  if (userIds.length === 0) return true
  const members = await db.query.teamMembers.findMany({
    where: (m, { and, eq }) => and(eq(m.teamId, teamId), inArray(m.userId, userIds)),
    columns: { userId: true },
  })
  return members.length === userIds.length
}
...
conversationsRoutes.post(
  '/direct',
  zValidator('json', createDirectConversationSchema),
  async (c) => {
    try {
      const user = c.get('user')
      const body = c.req.valid('json')
      ...
      // Verify both users are active members of the workspace
      const bothMembers = await areWorkspaceMembers(workspaceId, [userId1, userId2])
      if (!bothMembers) {
        return c.json({ success: false, error: 'Both users must be active workspace members' }, 403)
      }
...
conversationsRoutes.get('/', async (c) => {
  try {
    ...
    whereConditions.push(sql`${conversations.participants} @> ${JSON.stringify([userId])}::jsonb`)
    ...
    const sanitized = result.map((conv) => {
      ...
      const unreadCount = msgs.filter(
        (m) => new Date(m.timestamp) > readAt && m.senderId !== userId && !(m as any).isSystem // Optional: Don't count system messages?
      ).length
...
conversationsRoutes.post('/', zValidator('json', createConversationSchema), async (c) => {
  try {
    ...
    // Validate scope and creator membership
    if (teamId) {
      const creatorIsTeamMember = await areTeamMembers(teamId, [userId])
      if (!creatorIsTeamMember) {
        return c.json({ success: false, error: 'Forbidden: Not a team member' }, 403)
      }
    } else if (workspaceId) {
      const workspaceRole = await getUserWorkspaceRole(userId, workspaceId)
      if (!workspaceRole) {
    ...
    // Validate every participant is a member of the scope
    const membershipCheck = teamId
      ? await areTeamMembers(teamId, allParticipants)
      : workspaceId
        ? await areWorkspaceMembers(workspaceId, allParticipants)
        : false
    if (!membershipCheck) {
      return c.json(
        { success: false, error: 'All participants must be members of the workspace/team' },
        403
      )
    }
...
conversationsRoutes.post('/:id/messages', zValidator('json', createMessageSchema), async (c) => {
  try {
    ...
    const [data] = await db
      ...
      .leftJoin(
        workspaces,
        eq(sql`COALESCE(${conversations.workspaceId}, ${teams.workspaceId})`, workspaces.id)
      )
      .where(eq(conversations.id, conversationId))
      .limit(1)
    ...
    for (const targetId of otherParticipants) {
      await NotificationService.push(targetId, {
        ...
        link: workspaceSlug
          ? `/${workspaceSlug}/messages?userId=${conversationId}`
