...
import { checkFileSizeLimit, checkWorkspaceStorageLimit } from '../../lib/workspaceLimits'
import { isAllowedFile, sanitizeFileName, getFileExtension } from '../../lib/fileValidation'
...
const createTaskSchema = z.object({
  ...
  labels: z
    .array(z.string().or(z.object({ id: z.string() })))
    .optional()
    .nullable(),
...
tasksRoutes.get(
  ...
  async (c) => {
    try {
      ...
      console.log('🛡️ PERMISSIONS endpoint:', {
        userId,
        projectId,
        teamId,
        workspaceId,
...
tasksRoutes.get('/', zValidator('query', tasksQuerySchema), async (c) => {
  try {
    ...
    const { projectId, status, priority, assigneeId, isArchived, type, workspaceSlug } =
      c.req.valid('query')
                ...
                wheres.push(
                  exists(
                    db
                      ...
                      .from(projectMembers)
                      .where(
                        and(eq(projectMembers.projectId, p.id), eq(projectMembers.userId, userId))
                      )
                  )
                )
              }
    ...
    const dataWithCounts = await Promise.all(
      result.map(async (task) => {
        ...
        return {
          ...
          subtasksCompleted:
            task.subtasks?.filter((c: any) => c.isCompleted || c.status === 'done').length || 0,
...
tasksRoutes.post('/', zValidator('json', createTaskSchema), async (c) => {
  try {
    ...
    console.log('🛡️ CREATE task permission check:', {
      userId,
      ...
      workspaceRole,
      teamLevel,
      canCreateTasks: canCreateTasks(workspaceRole, teamLevel),
    })
    ...
    if (assignees.length > 0) {
      for (const assigneeId of assignees) {
        const isMember = await db.query.projectMembers.findFirst({
          where: (pm, { eq, and }) =>
            and(eq(pm.projectId, created.projectId), eq(pm.userId, assigneeId)),
        })
    ...
    if (body.labels && Array.isArray(body.labels)) {
      const labelIds = body.labels
        .map((label: string | { id: string }) => (typeof label === 'string' ? label : label.id))
        .filter(Boolean)
    ...
    if (workspaceId) {
      ...
      const [statusStage, workspace] = await Promise.all([
        created.status
          ? db.query.projectStages.findFirst({
...
tasksRoutes.patch('/:id', zValidator('json', updateTaskSchema), async (c) => {
  try {
    ...
    console.log('🛡️ UPDATE task permission check:', {
      taskId: id,
      userId,
      ...
      workspaceRole,
      teamLevel,
      canUpdateTasks: canUpdateTasks(workspaceRole, teamLevel),
    })
    ...
    if (!canProceed) {
      ...
      const isOnlyUpdatingCompletionState =
        changedKeys.length === 1 && changedKeys[0] === 'isCompleted'
    ...
    if (body.assignees !== undefined) {
      ...
      for (const assigneeId of newAssignees) {
        const isMember = await db.query.projectMembers.findFirst({
          where: (pm, { eq, and }) =>
            and(eq(pm.projectId, task.projectId), eq(pm.userId, assigneeId)),
        })
        if (!isMember) {
          await db.insert(projectMembers).values({
            projectId: task.projectId,
    ...
    if (workspaceId) {
      ...
      if (body.status !== undefined && body.status !== task.status) {
        ...
        const [oldStage, newStage] = await Promise.all([
          task.status
            ? db.query.projectStages.findFirst({
                where: (s, { eq }) => eq(s.id, task.status),
                columns: { name: true },
              })
            : null,
          updated.status
      ...
      // Only trigger task.updated if these specific fields changed
      if (
        (body.title !== undefined && body.title !== task.title) ||
        ...
        const statusStage = updated.status
          ? await db.query.projectStages.findFirst({
              where: (s, { eq }) => eq(s.id, updated.status!),
              columns: { name: true },
            })
          : null
    ...
    if (body.assignees) {
      for (const assigneeId of body.assignees) {
        const isMember = await db.query.projectMembers.findFirst({
          where: (pm, { eq, and }) =>
            and(eq(pm.projectId, updated.projectId), eq(pm.userId, assigneeId)),
        })
...
tasksRoutes.post('/:id/subtasks', async (c) => {
  try {
    ...
    await db.insert(taskActivityLog).values({
      taskId: id,
      userId:
        userId === 'temp-user-id' ? (await db.query.users.findFirst())?.id || 'temp-id' : userId,
...
tasksRoutes.patch('/:taskId/subtasks/:subtaskId', async (c) => {
  try {
    ...
    if (body.isCompleted !== undefined && body.isCompleted !== oldSubtask.isCompleted) {
      details = body.isCompleted
        ? `ukończył(a) podzadanie: ${updated.title} `
        : `przywrócił(a) podzadanie: ${updated.title} `
...
tasksRoutes.post('/:id/labels', async (c) => {
  try {
    ...
    if (!currentLabels.includes(labelId)) {
      await db
        .update(tasks)
        .set({ labels: [...currentLabels, labelId] })
...
tasksRoutes.post('/:id/upload', async (c) => {
  try {
    ...
    // Enforce workspace membership and upload permission
    const workspaceRole = await getUserWorkspaceRole(userId, workspaceId)
    if (!workspaceRole) {
      return c.json({ success: false, error: 'Forbidden: No active workspace access' }, 403)
    }
    if (!canUploadFiles(workspaceRole)) {
      return c.json(
        { success: false, error: 'Forbidden: You do not have permission to upload files' },
        403
      )
    }
    ...
    const fileExt = getFileExtension(safeName)

    // Validate file type, size, and storage quota
    if (!isAllowedFile(safeName, file.type || 'application/octet-stream')) {
      return c.json(
    ...
    const fileSizeCheck = await checkFileSizeLimit(workspaceId, file.size)
    if (!fileSizeCheck.allowed) {
      return c.json(
        { success: false, error: fileSizeCheck.error!.message, code: fileSizeCheck.error!.code },
        402
      )
    }

    const storageCheck = await checkWorkspaceStorageLimit(workspaceId, file.size)
    if (!storageCheck.allowed) {
      return c.json(
        { success: false, error: storageCheck.error!.message, code: storageCheck.error!.code },
        402
      )
    }
    ...
    const uploadUrl = await getUploadUrl(r2Key, file.type || 'application/octet-stream')
