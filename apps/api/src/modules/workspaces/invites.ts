...
workspaceInvitesRoutes.post('/:workspaceId/invites', async (c) => {
  ...
  try {
    ...
    if (!workspaceId.includes('-')) {
      // Assuming UUIDs contain dashes, slugs don't
...
workspaceInvitesRoutes.get('/:workspaceId/invites', async (c) => {
  ...
  try {
    ...
    const invites = await db.query.workspaceInvites.findMany({
      where: (wi, { eq, and }) => and(eq(wi.workspaceId, workspaceId), eq(wi.status, 'pending')),
  ...
  try {
    ...
    if (invite.allowedDomains && invite.allowedDomains.length > 0) {
      ...
      if (!invite.allowedDomains.includes(userDomain)) {
        return c.json(
          { error: `This invite is restricted to domains: ${invite.allowedDomains.join(', ')}` },
          403
        )
      }
    }
    ...
    await db.transaction(async (tx) => {
      ...
      const existing = await tx.query.workspaceMembers.findFirst({
        where: (wm, { eq, and }) =>
          and(eq(wm.userId, session.user.id), eq(wm.workspaceId, invite.workspaceId)),
      })
    ...
    triggerWebhook(
      'member.joined',
      { userId: session.user.id, role: invite.role, workspaceId: invite.workspaceId },
      invite.workspaceId
    )
