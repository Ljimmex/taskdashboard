...
import { sanitizeWhiteboardData } from '../../lib/sanitizeContent'
...
whiteboardsRoutes.get('/', async (c) => {
  try {
    ...
    const result = await db.query.whiteboards.findMany({
      where: (w, { eq, and }) => and(eq(w.workspaceId, workspaceId), eq(w.isArchived, false)),
      with: {
        creator: {
...
whiteboardsRoutes.post(
  '/',
  zValidator('json', createWhiteboardSchema, (result, c) => {
    if (!result.success) {
      console.error('Zod Validation Error on createWhiteboardSchema:', result.error.issues)
      return c.json(
  ...
  async (c) => {
    try {
      ...
      const newBoard: NewWhiteboard = {
        ...
        name: body.name,
        data: sanitizeWhiteboardData(body.data) || {},
        createdBy: user.id,
      }
