...
usersRoutes.post('/me/avatar', zValidator('form', avatarSchema), async (c) => {
  ...
  if (!file.type.startsWith('image/') || file.size > 5 * 1024 * 1024) {
    return c.json({ error: 'Only image files under 5 MB are allowed' }, 400)
  }
  ...
  const { error } = await supabase.storage.from('avatars').upload(versionedPath, file, {
    upsert: true,
  })
  ...
  const {
    data: { publicUrl },
  } = supabase.storage.from('avatars').getPublicUrl(versionedPath)
