...
const createFilterSchema = z.object({
  ...
  filters: z.record(z.string(), z.any()), // flexible JSON
  ...
  try {
    ...
    const filters = await db.query.savedFilters.findMany({
      where: (f, { eq, and, or }) => {
        ...
        conditions.push(or(eq(f.userId, userId), eq(f.isShared, true))!)
...
filtersRoutes.post('/:id/default', async (c) => {
  try {
    ...
    await db
      .update(savedFilters)
      .set({ isDefault: false })
      .where(and(eq(savedFilters.userId, userId), eq(savedFilters.workspaceId, filter.workspaceId)))
